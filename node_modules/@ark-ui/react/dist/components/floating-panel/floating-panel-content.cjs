"use client";
const require_compose_refs = require("../../utils/compose-refs.cjs");
const require_factory = require("../factory.cjs");
const require_presence_gate = require("../presence/presence-gate.cjs");
const require_use_presence_context = require("../presence/use-presence-context.cjs");
const require_use_floating_panel_context = require("./use-floating-panel-context.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/floating-panel/floating-panel-content.tsx
var FloatingPanelContent = (0, react.forwardRef)((props, ref) => {
	const floatingPanel = require_use_floating_panel_context.useFloatingPanelContext();
	const presence = require_use_presence_context.usePresenceContext();
	const mergedProps = (0, _zag_js_react.mergeProps)(floatingPanel.getContentProps(), presence.getPresenceProps(), props);
	const composedRefs = require_compose_refs.useComposedRefs(presence.ref, ref);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_presence_gate.PresenceGate, {
		presence,
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.div, {
			...mergedProps,
			ref: composedRefs
		})
	});
});
FloatingPanelContent.displayName = "FloatingPanelContent";
//#endregion
exports.FloatingPanelContent = FloatingPanelContent;
